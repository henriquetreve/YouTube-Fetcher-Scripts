    ...
    while len(video_data) < max_results:
        ...
        for i in range(0, len(video_ids), 50):
            ...
            for item in video_response['items']:
                ...
                if duration_seconds > 60:  # Only include videos longer than 60 seconds
                    video_format = 'Long video/standard YouTube video'
...
def main():
    ...
    if export_format == 'csv':
        filename = os.path.join(directory, f'youtube_longs_{sanitized_query}.csv') if directory else f'youtube_longs_{sanitized_query}.csv'
        export_to_csv(videos, filename)
    elif export_format == 'excel':
        filename = os.path.join(directory, f'youtube_longs_{sanitized_query}.xlsx') if directory else f'youtube_longs_{sanitized_query}.xlsx'
